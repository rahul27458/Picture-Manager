...
function check_videocomment(){
	var comment_text=document.getElementById("v_comment_text").value;
...
<!-- Check whether imagecomment is added -->
...
<!-- Set session -->
  ...
  <%
    ...
    %>
    <%! public boolean friend=true; %> <!-- to check whether people are friends or not  -->
    <!--header-->
<div class="header">
...
<div class="bar">
		...
		try{
			...
			// sql query to check if  a friends with b a->b then  b should be friends with a  b->a
			String sql="select user_id,friend_id from user_friend where user_id='"+user_id+"' and friend_id='"+friend_id+"' ";
			rs=st.executeQuery(sql);
			if(rs.next()){
				PostBean postBean=new PostBean();
				postBean.setUser_id(rs.getInt("user_id"));
				postBean.setFriend_id(rs.getInt("friend_id"));
				if(user_id==postBean.getUser_id() && friend_id==postBean.getFriend_id() && user_id!=friend_id)
				{ // members are friends
					
					%>
					<h2 style="position:relative;top:-160px;">Status:Friends</h2>
					<form method="GET" action="../AddFriend">
					<input type="hidden" name="user_id" id="user_id" value="<%=user_id%>">
					<input type="hidden" name="friend_id" id="friend_id" value="<%=friend_id%>">
				...
				// members are not friends and can add friend
				if(user_id!=friend_id){
				%>
				<form method="POST" action="../AddFriend?user_id=<%=user_id%>&friend_id=<%=friend_id%>">
		           <input type="submit" value="Add as Friend">
		          </form>
		        
				<% 
				friend=false; // not  a friend
				}
			}
			if(user_id==friend_id)
			{
...
<%
  if(true)
  {
	  ...
	  try{
		  PostBean postBean=new PostBean();
		  st=Database.dbconnect();
		  String sql="select profilepic,fname,dob,gender from user_detail where user_id="+friend_id;
		  rs=st.executeQuery(sql);
		  if(rs.next())
		  {
		  postBean.setFile_name(rs.getString("profilepic"));
		  postBean.setFname(rs.getString("fname"));
		  postBean.setDob(rs.getDate("dob"));
		  postBean.setGender(rs.getString("gender"));
		  String fileDes="";
		  if(postBean.getFile_name()!=null){
			fileDes="Your Profile picture";  
		 
		  %>
			<a href="/xyz/DisplayImages/?fileName=<%=postBean.getFile_name()%>" rel="prettyPhoto"><img src="/xyz/DisplayImages/?fileName=<%=postBean.getFile_name()%>" width="14%" height="15%" style="position:relative; left:20px;top:110px;" alt="<%=fileDes %>" /></a>
			  <h2 style="position:relative;top:10px;left:350px;">Member Name:<%=postBean.getFname()%></h2><br/>
			  <h2 style="position:relative;top:10px;left:350px;">Gender:<%=postBean.getGender()%></h2>
			  <h2 style="position:relative;top:10px;left:350px;">Date of Birth:<%=postBean.getDob() %></h2><br/>
	 ...
	 <%
			  ...
			  %>
			 <img src="" width="14%" height="15%" style="position:relative; left:20px;top:110px;" alt="no profile pic uploaded" />
			   <h2 style="position:relative;top:10px;left:350px;">Member Name:<%=postBean.getFname()%></h2></h2>
			  <h2 style="position:relative;top:15px;left:350px;">Date of Birth:<%=postBean.getDob() %></h2><br/>
...
<div class="right">
		...
		<div id="tabs-1">
									...
									<br>			
												...
												<% // check whether user_id and friend id match
												if(friend)
												{
					...
					{	
									...
									try
									{
												...
												String sqlret="select image_id,filename from imageupload where user_id='"+friend_id+"' and group_id is null ORDER BY image_id DESC";
											...
											for(PostBean postBean : dataList) 
											{
												...
												<a href="/xyz/DisplayImages/?fileName=<%=fileName%>" rel="prettyPhoto"><img src="/xyz/DisplayImages/?fileName=<%=postBean.getFile_name()%>" width="36%" height="30%" style="position:relative; left:100px;top:40px;" /></a><br/><br/><br/> 
										<br><br>
								...
								<%if(breport)// user has not reported 
									     ...
									     <textarea rows="4" cols="50" name="comment_text" id="<%=id%>">
                                                 </textarea> 
				...
				{			  int video_comment_id=0;
					...
					try
					{
									...
									String sqlret="select video_id,filename from videoupload where user_id='"+friend_id+"' and group_id is null ORDER BY video_id DESC";
									rs=st.executeQuery(sqlret); 
						...
						for(PostBean postBean : dataList) 
						{
								...
								<%if(breport)// user has not reported 
							      ...
							      <textarea rows="4" cols="50" name="v_comment_text" id="<%=video_comment_id%>">
                                                 </textarea> 
			...
			<div class="right_articles">
				...
				<h3>Your Groups</h3>
						...
						<% 
								...
								%>
								<!-- to porvide friend suggestions -->
<div id="suggestions">
<h2>People You May Know</h2>
<%if(true){
   ResultSet rs_suggest=null;
   Statement st_suggest=null;
   ResultSet rs_check=null; // to check whethe a particular user is a friend or not
   Statement st_check=null;
   List<PostBean> dataListSuggest=new ArrayList<PostBean>();
 
 try{
	 st_suggest=Database.dbconnect();
	 st_check=Database.dbconnect();
	 String sql="select rf.friend_id,ru.fname from user_friend rf,user_detail ru where rf.user_id='"+friend_id+"' and rf.friend_id=ru.user_id and rf.friend_id not in (select friend_id from user_friend where user_id='"+user_id+"') and '"+user_id+"' <> '"+friend_id+"'";
	
	 rs_suggest=st_suggest.executeQuery(sql);
	
	 
	 // check whether both user_id and friend_id are not the same people
	
		 
	 
	 while(rs_suggest.next())
	 {
		 PostBean postBeanSuggest=new PostBean();
		 
		 postBeanSuggest.setUser_id(rs_suggest.getInt("friend_id"));
		 postBeanSuggest.setFname(rs_suggest.getString("fname"));
		 
		 
		 dataListSuggest.add(postBeanSuggest);
	 }
	 for(PostBean postBeanSuggest : dataListSuggest)
	 { 
		if(user_id!= postBeanSuggest.getUser_id())
		{
		 %>
		  
		  <a href="see_profile.jsp?friend_id=<%=postBeanSuggest.getUser_id()%>&user_id=<%=user_id%>"><h2><%=postBeanSuggest.getFname() %></h2></a>
